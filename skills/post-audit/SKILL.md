...
- explicit ruling: `won't-fix` · `withdrawn` · `fix in lane` / `fix-in-lane` · `landed` · `closed` (these are **rulings, past tense** — write them only when true; "to be landed later" is not a ruling and must not appear on a declaration line).
...
**Ids and severity:** single-letter ids `C/H/B/S/I/M`-n are severity-bearing (an `M-` id with no severity word counts as Medium — fail-closed; `L-`/`F-` need a severity word to enforce). Multi-letter prefixes (`INV-`, `SEC-`, `TI-`, …) are namespaces — enforced only when a declaration line carries a severity word. Uppercase `MED/IMP/CRIT/BLK/MIN/LOW` count as severity words; "high confidence" does not. **`MUT-n` is reserved for mutation probes and is never a finding id** — do not name findings `MUT-*`.
